...
// FIX: Use legacy import to resolve "deleteAsync" deprecation error
import * as FileSystem from 'expo-file-system/legacy'; 
import AsyncStorage from '@react-native-async-storage/async-storage'; 

import { Trash2, ChevronRight, Moon, Image as ImageIcon, EyeOff } from 'lucide-react-native';
...
const Settings = () => {
  ...
  const handleClearCache = async () => {
    try {
      Alert.alert(
        "Clear Cache",
        "Are you sure? This will free up space but images may take longer to load next time.",
        [
          ...
          { 
            ...
            style: "destructive",
            onPress: async () => {
              // FIX: Using legacy import method which supports deleteAsync
  ...
  const SettingToggleRow = ({ icon: Icon, title, subtitle, value, onValueChange }) => (
    <View style={{
    ...
    <View style={{ flex: 1, backgroundColor: '#000', paddingTop: insets.top }}>
      ...
      <ScrollView
        ...
        contentContainerStyle={{ 
          paddingBottom: tabBarHeight + 50, // Ensures content rises above bottom nav
      ...
      >
        ...
        <View style={{ marginBottom: 20 }}>
          <Text style={{ color: '#888', fontSize: 14, marginBottom: 10, textTransform: 'uppercase' }}>
            Content
          </Text>
        ...
        <View style={styles.section}>
          <Text>Playback Section...</Text>
        </View>
        <View style={styles.section}>
          <Text>Haptics Section...</Text>
        </View>
        <View style={styles.section}>
