...
const WatchListPage = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // ✅ FIXED: Changed 'watched' to 'history' to sync with DetailPage
  ...
  const handleRemove = async (id: number, type: 'watchlist' | 'artist' | 'watched') => {
    if (type === 'watchlist') {
        ...
        setWatched(newList);
        // ✅ FIXED: Changed 'watched' to 'history'
  ...
  const renderCard = ({ item }: { item: any }) => {
    ...
    const itemType = activeTab === 0 ? 'watchlist' : activeTab === 1 ? 'artist' : 'watched';
  ...
  const currentList = activeTab === 0 ? watchlist : activeTab === 1 ? artists : watched;
...
const styles = StyleSheet.create({
  ...
  blurContainer: { ...StyleSheet.absoluteFillObject, borderRadius: 22, overflow: 'hidden' },
